 ...
 * @param  [type] $out_template [description]
...
function tlist() 
{
    ...
    while (list($category, $dcount) = sql_fetch_row($result)) 
    {
        ...
        if ($category == $cate)
        {
            echo '<i class="fa fa-folder-open fa-2x text-muted align-middle mr-2"></i><strong class="align-middle">'.language::aff_langue($category).'<span class="badge badge-secondary ml-2 float-right my-2">'.$dcount.'</span></strong>';
        }
        else 
        {
            $category2 = urlencode($category);
            echo '<a href="download.php?dcategory='.$category2.'&amp;sortby='.$sortby.'"><i class="fa fa-folder fa-2x align-middle mr-2"></i><span class="align-middle">'.language::aff_langue($category).'</span></a><span class="badge badge-secondary ml-2 my-2 float-right">'.$dcount.'</span>';
        }
 ...
 * @param  [type] $dcategory   [description]
 * @param  [type] $sortby      [description]
 * @param  [type] $fieldname   [description]
 * @param  [type] $englishname [description]
 * @return [type]              [description]
 */
 ...
 * @param  [type] $did          [description]
 * @param  [type] $ddescription [description]
 * @param  [type] $dcounter     [description]
 * @param  [type] $dfilename    [description]
 * @param  [type] $aff          [description]
 * @return [type]               [description]
 */
...
function listdownloads ($dcategory, $sortby, $sortorder) 
{
    ...
    echo '
    <table class="table table-hover mb-3 table-sm" id ="lst_downlo" data-toggle="table" data-striped="true" data-search="true" data-show-toggle="true" data-show-columns="true" data-mobile-responsive="true" data-buttons-class="outline-secondary" data-icons-prefix="fa" data-icons="icons">';
    ...
    echo '<div class="mt-3"></div>'.pagination::paginate_single('download.php?dcategory='.$dcategory.'&amp;sortby='.$sortby.'&amp;sortorder='.$sortorder.'&amp;page=', '', $nbPages, $current, $adj=3, '', $page);
}
...
function main() 
{
    ...
    $dcategory = hack::remove(stripslashes(htmlspecialchars(urldecode($dcategory), ENT_QUOTES, cur_charset))); // electrobug
    
    $dcategory = str_replace("&#039;", "\'", $dcategory);
    
    $sortby = hack::remove(stripslashes(htmlspecialchars(urldecode($sortby), ENT_QUOTES, cur_charset))); // electrobug
...
function broken($did) 
{
    ...
    if ($user) 
    {
        if ($did) 
        {
            ...
            $message = $nuke_url."\n\n".translate("Téléchargements")." ID : $did\n\n".translate("Auteur")." $cookie[1] / IP : ".ip::get();
            
            mailler::send_email($notify_email, translate("Rapporter un lien rompu"), $message, $notify_from , false, "text");
