...
function NoMoveCategory($catid, $newcat) 
{
       ...
       <form action="admin.php" method="post">
          ...
          while(list($newcat, $title) = sql_fetch_row($selcat)) {
             echo '
                   <option name="newcat" value="'.$newcat.'">'.language::aff_langue($title).'</option>';
          }
...
function displayStory ($qid) 
{
       ...
       publication::code_aff('<h4>'.$subject.$topiclogo.'</h4>','<div class="text-muted">'.metalang::meta_lang($story).'</div>', metalang::meta_lang($bodytext), "");
       ...
       echo tiny::aff_editeur('hometext', '');
...
function previewStory($qid, $uid, $author, $subject, $hometext, $bodytext, $topic, $notes, $catid, $ihome, $members, $Mmembers, $dd_pub, $fd_pub, $dh_pub, $fh_pub, $epur) 
{
       ...
       publication::code_aff('<h3>'.$subject.$topiclogo.'</h3>', '<div class="text-muted">'.metalang::meta_lang($hometext).'</div>', metalang::meta_lang($bodytext), metalang::meta_lang($notes));
...
function editStory ($sid) 
{
       ...
       echo '<hr />'.language::aff_local_langue('<label class="col-form-label">'.adm_translate("Langue de Prévisualisation").'</label>','','local_user_language');
       ...
       echo publication::code_aff('<h3>'.$subject.$topiclogo.'</h3>', '<div class="text-muted">'.$hometext.'</div>', $bodytext, $notes);
       ...
       while(list($topicid, $topics, $topicadmin) = sql_fetch_row($toplist)) {
          ...
          if ($affiche) {
             ...
             echo '
                   <option value="'.$topicid.'" '.$sel.'>'.language::aff_langue($topics).'</option>';
             $sel = '';
          }
       }
       ...
       echo tiny::aff_editeur("hometext", "true");
       ...
       echo tiny::aff_editeur("bodytext", "true");
       ...
       publication::publication(-1, $fd_pub, -1, $fh_pub, $epur);
...
function previewAdminStory($subject, $hometext, $bodytext, $topic, $catid, $ihome, $members, $Mmembers, $dd_pub, $fd_pub, $dh_pub, $fh_pub, $epur) 
{
    ...
    //   admindroits($aid,$f_meta_nom); // à voir l'intégration avec les droits sur les topics ...
       ...
       publication::code_aff('<h3>'.$subject.$topiclogo.'</h3>', '<div class="text-muted">'.$hometext.'</div>', $bodytext, '');
