          ...
          if ($gpA) {
             ...
             <div class="row border rounded ml-1 p-2 px-0 mb-2 w-100">
                <div id="bloc_gr_'.$gp.'" class="col-lg-5">
                   ...
                   <h4 class="my-2 text-muted">'.language::aff_langue($gp_name).'</h4>
                   <p class="text-muted">'.language::aff_langue($gp_description);
...
function membre_add($gp) 
{
       ...
       echo auto::auto_complete_multi('membre', 'uname', 'users', 'luname', 'inner join users_status on users.uid=users_status.uid WHERE users.uid<>1 and groupe not regexp \'[[:<:]]'.$gp.'[[:>:]]\'');
...
function forum_groupe_delete($groupe_id) 
{
        ...
        global $aid; 
        logs::Ecr_Log('security', "DeleteForumWS($forum_id) by AID : $aid", '');
}
