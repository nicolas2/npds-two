...
function sousrub_select($secid) 
{
       ...
       while(list($rubid, $rubname) = sql_fetch_row($result)) {
          ...
          $tmp .= '
                <optgroup label="'.language::aff_langue($rubname).'">';
...
function sections() 
{
       ...
       if ($nb_rub > 0) {
          ...
          while (list($rubid, $rubname, $enligne, $ordre) = sql_fetch_row($result)) {
             ...
             if ($enligne == 0) { 
                $online = '<span class="badge badge-danger ml-1">'.adm_translate("Hors Ligne").'</span>'; 
             } else if ($enligne == 1) { 
                $online = '<span class="badge badge-success ml-1">'.adm_translate("En Ligne").'</span>'; 
             }
                  ...
                  echo '</span>
                   ...
                   if (sql_num_rows($result3) > 0) {
                      ...
                      while (list($artid, $title) = sql_fetch_row($result3)) {
                         ...
                         echo '
                         <li class="list-group-item list-group-item-action d-flex"><span class="ml-4">'.language::aff_langue($title).'</span>
                            <span class="ml-auto">
                         ...
                         if (($droit_pub == 7) or ($droit_pub == 4))
                            echo '
                               <a href="admin.php?op=secartdelete&amp;artid='.$artid.'" class="text-danger" title="'.adm_translate("Supprimer").'" data-toggle="tooltip"><i class="far fa-trash-alt fa-lg"></i></a>';
    ...
    // ca c'est pas bon incomplet
       ...
       if ($radminsuper == 1) {
          ...
          while(list($artid, $secid, $title, $content, $author) = @sql_fetch_row($result)) {
             echo '<li class="list-group-item list-group-item-action" >'.language::aff_langue($title).'<span class="d-block float-right"><a href="admin.php?op=secartupdate&amp;artid='.$artid.'"><i class="fa fa-edit fa-lg"></i>'.adm_translate("Editer").'</a></span><br /><span class="text-muted"><i class="fa fa-user fa-lg mr-1"></i>['.$author.']</span>';
          }
       } else {
...
function new_rub_section($type) 
{
       ...
       if ($type == 'sec') {
          ...
          while (list($rubid, $rubname) = sql_fetch_row($result)) {
             echo '
                      <option value="'.$rubid.'">'.language::aff_langue($rubname).'</option>';
          }
       ...
       while (list($rubid, $rubname, $enligne, $ordre) = sql_fetch_row($result)) {
          ...
          echo '
             <div class="list-group-item">
                <a class="arrow-toggle text-primary" data-toggle="collapse" data-target="#lst_'.$rubid.'" ><i class="toggle-icon fa fa-caret-down fa-lg"></i></a>&nbsp;'.language::aff_langue($rubname).'<span class="badge badge-'.$cla.' float-right">'.$online.'</span>
             </div>';
          ...
          if (sql_num_rows($result2) > 0) {
             ...
             while (list($secid, $secname) = sql_fetch_row($result2)) {
                echo '
                <li class="list-group-item"><strong class="ml-3" title="'.adm_translate("sous-rubrique").'" data-toggle="tooltip">'.language::aff_langue($secname).'</strong></li>';
                $result3 = sql_query("SELECT artid, title FROM ".$NPDS_Prefix."seccont WHERE secid='$secid' ORDER BY ordre");
                ...
                if (sql_num_rows($result3) > 0) {
                   while (list($artid, $title) = sql_fetch_row($result3)) {
                      ...
                      echo '
                <li class="list-group-item list-group-item-action"><span class="ml-2">'.language::aff_langue($title).'</span>';
...
function sectionedit($secid) 
{
       ...
       if ($radminsuper == 1) {
          ...
          echo language::aff_langue($rubname);
       }
...
function sectionmake($secname, $image, $members, $Mmembers, $rubref, $introd) 
{
       ...
       $introd = stripslashes(str::FixQuotes($introd));
...
function publishrights($author) 
{
       ...
       if ($radminsuper != 1) {
          Header("Locatio n: admin.php?op=sections");
       }
             ...
             <table data-classes="" data-toggle="table" data-striped="true" data-icons-prefix="fa" data-icons="icons">
                <thead>
                   <tr>
                      ...
                      echo language::aff_langue($rubname).'</th>
          ...
          while(list($secid, $secname) = sql_fetch_row($result2)) {
             ...
             echo '
                   <tr>
                      ...
                      <td>'.language::aff_langue($secname).'</td>
