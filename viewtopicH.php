...
function aff_pub($lock_state, $topic, $forum, $post, $bouton) {
   ...
   if ($lock_state==0) {
      ...
      $ibid = '<i class="fa fa-lock fa-lg text-danger" title="'.translate("Ce sujet est verrouillé : il ne peut accueillir aucune nouvelle contribution.").'" data-toggle="tooltip"></i>&nbsp;';
   }
   return $ibid;
}
...
function aff_pub_in($lock_state, $topic, $forum, $post) {
   ...
   $ibid='';
   if ($lock_state==0)
      $ibid = '<a class="mr-3" href="replyH.php?topic='.$topic.'&amp;forum='.$forum.'&amp;post='.$post.'" title="'.translate("Répondre").'" data-toggle="tooltip"><span class="d-none d-md-inline"></span><i class="fa fa-reply mr-2"></i><span class="d-none d-md-inline">'.translate("Répondre").'</span></a>';
   return $ibid;
}
...
$r_to='';$n_to='';
  ...
  echo '
   ...
   for ($i = 0; $i < $ibidcountmod; $i++) {
      ...
      echo '<a href="user.php?op=userinfo&amp;uname='.$moderator[$i].'"><img class=" img-thumbnail img-fluid n-ava-small mr-1 mb-1" src="'.$imgtmp.'" alt="'.$modera['uname'].'" title="'.translate("Modéré par : ").' '.$modera['uname'].'" data-toggle="tooltip" /></a>';
   }
   echo '
            </div>
         </div>
   ...
   if ($forum_access!=9) {
      ...
      if ((($Mmod) or ($original_poster==$userdata[0])) and (!$lock_state)) {
         ...
         echo '
         <p><a href="viewforum.php?forum='.$forum.'&amp;topic_id='.$topic.'&amp;topic_title='.rawurlencode($topic_subject).'&amp;op=solved&amp;sec_clef='.$sec_clef.'"><i class="fa fa-lock fa-2x align-middle mr-1"></i>'.translate("Résolu").'</a></p>';
         unset($sec_clef);
      }
   }
...
<a name="botofpage"></a>';
   ...
   if ((($Mmod) and ($forum_access!=9)) or ($adminforum==1)) {
      ...
      echo '
            <li class="nav-item">
               <a class="nav-link" role="button" href="topicadmin.php?mode=move&amp;topic='.$topic.'&amp;forum='.$forum.'&amp;arbre=1"><i class="fa fa-share fa-lg d-md-none" title="'.translate("Déplacer ce sujet").'" data-toggle="tooltip" ></i><span class="ml-2 d-none d-md-inline">'.translate("Déplacer ce sujet").'</span></a>
            </li>
            <li class="nav-item">
