    ...
    or (!$SuperCache)) 
{
    ...
    if ($bodytext == '')
    {
        $bodytext = metalang::meta_lang($hometext.'<br />'.$notes);
    }
    else
    {
        $bodytext = metalang::meta_lang($hometext.'<br />'.$bodytext.'<br />'.$notes);
    }
    ...
    if ($catid != 0) 
    {
        ...
        $title = '<a href="'.site_url('index.php?op=newindex&amp;catid='.$catid).'"><span>'.language::aff_langue($title1).'</span></a> : '.$title;
    }
    ...
    {
        $boxstuff .= '
        <li><a href="'.site_url($url).'" target="new"><span>'.$name.'</span></a></li>';
    }

    $boxstuff .= '
    </ul>
    <ul>
        <li><a href="'.site_url('search.php?topic='.$topic).'" >'.translate("En savoir plus à propos de").' '.language::aff_langue($topictext).'</a></li>
        <li><a href="'.site_url('search.php?member='.$informant).'" >'.translate("Article de").' '.$informant.'</a></li>
    </ul>
    <div class="">'.translate("L'article le plus lu à propos de").'&nbsp;&nbsp;'.language::aff_langue($topictext).' :</div>';

    $xtab = news::news_aff("big_story", "WHERE topic=$topic", 0, 1);
    ...
    <div class="">'.translate("Les dernières nouvelles à propos de").' '.language::aff_langue($topictext).' :</div>';

    if (!$archive)
    {
        $xtab = news::news_aff("libre","WHERE topic=$topic AND archive='0' ORDER BY sid DESC LIMIT 0,5", 0, 5);
    }
    else
    {
        $xtab = news::news_aff("archive","WHERE topic=$topic AND archive='1' ORDER BY sid DESC LIMIT 0,5", 0, 5);
    }
    ...
    while (($story_limit<5) and ($story_limit<sizeof($xtab))) 
    {
        ...
        $boxstuff .= '
        <li><a href="'.site_url('article.php?sid='.$sid1.'&amp;archive='.$archive).'" >'.language::aff_langue(stripslashes($title1)).'</a></li>';
    }

    $boxstuff .= '
    </ul>
    <p align="center">
        <a href="'.site_url('print.php?sid='.$sid.'&amp;archive='.$archive).'" ><i class="fa fa-print fa-lg mr-1" title="'.translate("Page spéciale pour impression").'" data-toggle="tooltip"></i></a>
        <a href="'.site_url('friend.php?op=FriendSend&amp;sid='.$sid.'&amp;archive='.$archive).'"><i class="fa fa-lg fa-at" title="'.translate("Envoyer cet article à un ami").'" data-toggle="tooltip"></i></a>
    </p>';
