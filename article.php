...
if (!isset($sid) && !isset($tid))
   ...
   if (($cache_obj->genereting_output==1) or ($cache_obj->genereting_output==-1) or (!$SuperCache)) {
      ...
      if ($catid != 0) {
         ...
         $title = '<a href="'.site_url('index.php?op=newindex&amp;catid='.$catid).'"><span>'.aff_langue($title1).'</span></a> : '.$title;
      }
      ...
      <ul>
         <li><a href="'.site_url('search.php?topic='.$topic).'" >'.translate("En savoir plus à propos de").' '.aff_langue($topictext).'</a></li>
      ...
      $boxstuff .= '
      <ul>
         <li><a href="'.site_url('article.php?sid='.$topstory).'" >'.aff_langue($ttitle).'</a></li>
      </ul>
      ...
      while (($story_limit<5) and ($story_limit<sizeof($xtab))) {
         ...
         $boxstuff .= '
         <li><a href="'.site_url('article.php?sid='.$sid1.'&amp;archive='.$archive).'" >'.aff_langue(stripslashes($title1)).'</a></li>';
      }
