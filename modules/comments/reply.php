...
if (isset($submitS)) 
{
    ...
    if ($stop != 1) 
    {
        ...
        // anti flood
        forumaddon::anti_flood($Mmod, $anti_flood, $poster_ip, $userdata, $gmt);
        //anti_spambot
        ...
        if ($formulaire != '')
        {
            include ("modules/comments/sform/comments_extender.php");
        }
...
else 
{
    ...
    if ($allow_to_reply) 
    {
        if (isset($submitP)) 
        {
            ...
            {
                $userdata = array('uid' => 1);
                $userdata = auth::get_userdata($userdata['uid']);
            }

            $theposterdata = auth::get_userdata_from_id($userdata['uid']);
            $messageP = $message;
            ...
            if ($smilies) 
            {
                if ($theposterdata['user_avatar'] != '') 
                {
                    ...
                    echo '
                    <a style="position:absolute; top:1rem;" tabindex="0" data-toggle="popover" data-html="true" data-title="'.$theposterdata['uname'].'" data-content=\''.forumauth::member_qualif($theposterdata['uname'], $theposterdata['posts'], $theposterdata['rank']).'\'><img class=" btn-secondary img-thumbnail img-fluid n-ava" src="'.$imgtmp.'" alt="'.$theposterdata['uname'].'" /></a>';
                }
            }
    ...
    if ($allow_to_reply) 
    {
        ...
        if (sql_num_rows($result)) 
        {
            ...
            while($myrow = sql_fetch_assoc($result)) 
            {
                ...
                echo forumauth::member_qualif($posterdata['uname'], $posts, $posterdata['rank']);
                ...
                echo "&nbsp;".translate("Posté : ").date::convertdate($myrow['post_time']);
                echo '<hr /> ';
