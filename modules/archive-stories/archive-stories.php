    ...
    or (!$SuperCache)) 
{
    ...
    if (!isset($count)) 
    {
        $result0 = cache::Q_select("SELECT COUNT(sid) AS count FROM ".$NPDS_Prefix."stories WHERE archive='$arch'",3600);
        $count = $result0[0];
        $count = $count['count'];
    }
    ...
    if ($arch == 0) 
    {
        $xtab = news::news_aff("libre", "WHERE archive='$arch' ORDER BY sid DESC LIMIT $start,$maxcount", $start, $maxcount);
    }
    else
    {
        $xtab = news::news_aff("archive", "WHERE archive='$arch' ORDER BY sid DESC LIMIT $start,$maxcount", $start, $maxcount);
    }
    ...
    echo pagination::paginate('modules.php?ModPath=archive-stories&amp;ModStart=archive-stories&amp;start=', '&amp;count='.$count, $nbPages, $current, 1, $maxcount, $start);
    echo '</div>';
}
