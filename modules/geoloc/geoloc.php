...
while ($row = sql_fetch_array($result)) 
{
    ...
    if($r == 0) 
    {
        ...
        {
            $acg++; $acng++;
            $test_ip .= '<a title="IP non géoréférencé en ligne" class="sb_ano list-group-item  py-1">IP '.$acng.' <small>'.urldecode($session_host_addr).'</small></a>';
        }
    }
...
foreach ($fond_provider as $v) 
{
    ...
    {
        case '0': 
            $affi .= '<optgroup label="OpenStreetMap">';
        break;
        
        case '1': 
            $affi .= '<optgroup label="Stamen">';
        break;
        
        case '4': 
            $affi .= '<optgroup label="NASA">';
        break;
        
        case '5': 
            if ($api_key_mapbox ==! '') 
            {
                $affi .= '<optgroup label="Mapbox">';
            } elseif ($api_key_bing ==! '') 
            {
                $affi .= '<optgroup label="Bing maps">';
            } 
        break;
