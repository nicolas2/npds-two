 ...
 * le géoréférencement des membres sur une géolocalisation exacte 
 * réalisé par l'utilisateur
 * la geolocalisation instantanée est réalisé par les api html5 de géolocalisation 
 * (!! elle  n'est disponible que pour un site en https)
 */
...
{
    $f_geomod = hack::remove($_GET['mod']);
}

if (array_key_exists('uid', $_GET)) 
{
    $f_uid = hack::remove($_GET['uid']);
}
...
//==> membres du site
$total_membre = sql_num_rows($membre);
    
$k = 0;
$us_visit = array();
$cont_json = ''; 
$cont_geojson = ''; 
$mbr_geo_off = 'var mbr_features=['; 
$mbr_geo_off_v = '';
...
while ($row = sql_fetch_array($membre)) 
{
    ...
    if ($us_lat != '') 
    {
        ...
        if($mark_typ !== 1) 
        {
            $ic_sb_mbg ='<i style=\"color:'.$mbg_f_co.'; opacity:0.4;\" class=\"fa fa-'.strtolower($f_mbg).' fa-lg mr-1\"></i>';
        }
        else 
        {
...
// construction du fichier json
$f = fopen("modules/".$ModPath."/storage/json/user.json", "w");
...
$result = sql_query ('SELECT * FROM '.$NPDS_Prefix.'session s LEFT JOIN '.$NPDS_Prefix.'users u ON s.username = u.uname LEFT JOIN '.$NPDS_Prefix.'users_extend ue ON u.uid = ue.uid');
...
while ($row = sql_fetch_array($result)) 
{
    ...
    {
        $av_ch = $users_user_avatar;
    }
    else
    {
        $av_ch = 'assets/images/forum/avatar/'.$users_user_avatar;
    };
    ...
    { 
        ...
        if($mark_typ !== 1) 
        {
            $ic_sb_mbgc = '<i style=\"color:'.$mbgc_f_co.';\" class=\"fa fa-'.strtolower($f_mbg).' fa-lg animated faa-pulse mr-1\"></i>';
        }
        else 
        {
    ...
    if($r == 0) 
    {
        if ($users_uname != $session_user_name) 
        {
            $acg++; 
            $acng++;
            //$test_ip .='<a title="IP non géoréférencé en ligne" class="sb_ano list-group-item  py-1">IP 777'.$acng.' <small>'.$session_host_addr.'</small></a>';
            $test_ip .='<a title="IP non géoréférencé en ligne" class="sb_ano list-group-item  py-1">IP Anonyme '.$acng.' : <small>'.urldecode($session_host_addr).'</small></a>';
        }
    }
...
<script type="text/javascript">
    //<![CDATA[
        ...
        if (!$("link[href=\'modules/geoloc/assets/css/geoloc_style.css\']").length)
            $("head link[rel=\'stylesheet\']").last().after("<link rel=\'stylesheet\' href=\'/modules/geoloc/assets/css/geoloc_style.css\' type=\'text/css\' media=\'screen\'>");
        ...
        $(function () {
            ...
            var src_countries = new ol.source.Vector({
                url: "/modules/geoloc/storage/json/countries.json",
...
foreach ($fond_provider as $v) 
{
    ...
    $affi .= '<option '.$sel.' value="'.$v[0].'">'.$v[1].'</option>';
    ...
    switch($j)
    {
        ...
        case '7': 
            if($api_key_mapbox = '' and $api_key_bing ==! '') 
            {
                $affi .= '
                </optgroup>'; 
            }
...
$affi .= '
    ...
    <ul class="nav nav-tabs mt-4">
        ...
        <li class="nav-item"><a id="aide-tab" class="nav-link" href="modules/geoloc/views/doc/aide_geo.html" data-target="#aide" data-toggle="tab_ajax"><span class="d-sm-none"><i class=" fa fa-globe fa-lg mr-2"></i><i class=" fa fa-question fa-lg"></i></span><span class="d-none d-sm-inline">'.geoloc_translate("Aide").'</span></a></li>';
