 ...
 * @param [type] $hlpfile [description]
 */
function GraphicAdmin($hlpfile) 
{
    ...
    // recupérations des états des fonctions d'ALERTE ou activable et maj 
    // (faire une fonction avec cache court dev ..)
    ...
    if($versus_info[1] == NpdsTwo::VERSION_SUB and $versus_info[2] == NpdsTwo::VERSION_NUM)
    {
        sql_query("UPDATE ".$NPDS_Prefix."fonctions SET fetat='1', fretour='', fretour_h='Version Npds Two ".NpdsTwo::VERSION_SUB." ".NpdsTwo::VERSION_NUM."', furlscript='' WHERE fid='36'");
    }
    else
    {
        sql_query("UPDATE ".$NPDS_Prefix."fonctions SET fetat='1', fretour='N', furlscript='data-toggle=\"modal\" data-target=\"#versusModal\"', fretour_h='Une nouvelle version Npds Two est disponible !<br />".$versus_info[1]." ".$versus_info[2]."<br />Cliquez pour télécharger.' WHERE fid='36'");
    }
    ...
    // selection de fonctions actives ayant une interface graphique de 
    // premier niveau et dont l'administrateur connecté en posséde les droits d'accès
    ...
    while($SAQ = sql_fetch_assoc($R)) 
    {
        ...
        if ($SAQ['fcategorie'] == 6 
            or ($SAQ['fcategorie'] == 9 
            and strstr($SAQ['furlscript'], "op=Extend-Admin-SubModule"))) 
        {
 ...
 * @param  [type] $deja_affiches [description]
 * @return [type]                [description]
 */
function adminMain($deja_affiches) 
{
    ...
    if ($nbre_articles) 
    {
        ...
        while( (list($sid, $title, $hometext, $topic, $informant, $time, $archive, $catid, $ihome) = sql_fetch_row($result)) and ($i < $admart) ) 
        {
            ...
            else 
            {
                if ($affiche) 
                {
                    ...
                    if($ihome== 1) 
                    {
                         echo '<br /><small><span class="badge badge-secondary" title="'.adm_translate("Catégorie").'" data-toggle="tooltip">'.language::aff_langue($cat_title).'</span> <span class="text-danger">non publié en index</span></small>';
                    }
                    else
                    {
                        if($catid>0){
                            echo '<br /><small><span class="badge badge-secondary" title="'.adm_translate("Catégorie").'" data-toggle="tooltip"> '.language::aff_langue($cat_title).'</span> <span class="text-success"> publié en index</span></small>';
                        }
                    }
                } 
                else 
            ...
            {
                echo '</td>
                <td>'.$topictext.'<a href="'.site_url('index.php?op=newtopic&amp;topic='.$topic).'" class="tooltip">'.language::aff_langue($topictext).'</a>';
            }
        ...
        <div class="d-flex my-2 justify-content-between flex-wrap">
            ...
            echo pagination::paginate(site_url('admin.php?op=suite_articles&amp;deja_affiches='), '', $nbPages, $current, 1, $admart, $start);
