...
//            return ($this->pretty_Size($this->Size));
   ...
   function Affiche_Extention($Format) {
      ...
      switch ($Format) {
         case "IMG":
            if ($ibid=theme_image("upload/file_types/".$this->Extention.".gif")) {$imgtmp=$ibid;} else {$imgtmp="assets/images/upload/file_types/".$this->Extention.".gif";}
            if (@file_exists($imgtmp)) return '<img src="'.$imgtmp.'" />'; else return '<img src="assets/images/upload/file_types/unknown.gif" />';
         break;
         case "webfont":
            return '
            <span class="fa-stack">
